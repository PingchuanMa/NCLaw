...
from nclaw.render import get_slope_camera
...
def render(cfg):
    ...
    rotation = R.from_euler('x', -30, degrees=True).as_matrix()

    plotter = pv.Plotter(off_screen=True, window_size=(cfg.render.width, cfg.render.height))
    plotter.camera_position = get_slope_camera()

    ckpt_paths = list(sorted(state_root.glob('*.pt')))
    for i, path in enumerate(tqdm(ckpt_paths, desc=render_type)):
        ...
        x = ckpt['x'].cpu().detach().numpy()
        x = (x - np.array(cfg.env.blob.shape.center)) @ rotation + np.array(cfg.env.blob.shape.center)
        x_sections = np.split((x - 0.5) * scale + 0.5, np.cumsum(ckpt['sections']), axis=0)
