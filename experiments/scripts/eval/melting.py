...
def main():
    ...
    for epoch in EPOCHS:
        ...
        for blob in ['icecream', 'chocolate']:
            for i, material in enumerate(['plasticine', 'sand']):
                args = args | {
                    f'env/blob/material/elasticity@env.{blob}.material{i}.elasticity': elasticity,
                    f'env/blob/material/plasticity@env.{blob}.material{i}.plasticity': plasticity,
                    f'env.{blob}.material{i}.ckpt': Path(material) / 'train' / f'{elasticity}-{plasticity}' / 'ckpt' / f'{epoch:04d}.pt',
                }
    ...
    for args in gt_args_list + epoch_args_list:
        ...
        if not extra_args.skip:
            base_cmds = [python_path, root / 'eval-melting.py']
