...
def main():
    ...
    for epoch in EPOCHS:
        ...
        args = subargs | {
            ...
            'env/blob/material/elasticity@env.water.material.elasticity': elasticity,
            'env/blob/material/plasticity@env.water.material.plasticity': plasticity,
            'env.water.material.ckpt': Path('water') / 'train' / f'{elasticity}-{plasticity}' / 'ckpt' / f'{epoch:04d}.pt',
            'name': exp_name
        }
        if epoch == 0:
            args['render.mpm_mul'] = 0.0
        else:
            args['render.mpm_mul'] = 5.5
