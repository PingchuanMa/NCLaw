    ...
    for env in ENVS:
        ...
        for seed in SEEDS:
            ...
            subargs = base_args | {
                'env': env,
                'env/blob/vel': 'random',
                'env.blob.vel.seed': seed,
                'render': RENDER,
                'sim': quality,
            }

            args = subargs | {
                'name': Path(env) / gt_mode / f'{seed:04d}'
            }
            ...
            for elasticity, plasticity in [
                ...
                for epoch in EPOCHS:
                    ...
                    exp_name = name / gt_mode / f'{seed:04d}' / f'{epoch:04d}'
                ...
                for render in extra_args.video:
                    ...
                    cat_videos(videos, root / 'log' / name / 'videos' / f'{gt_mode}-{seed:04d}-{render}.mp4')
